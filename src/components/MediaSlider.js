...
const MediaSlider = ({media, categoryTitle, mediaType, lessSlides}) => {
    ...
    return(
        <section className='slider'>
            <Fade><h4 className='slider_title my-4'>{categoryTitle}</h4></Fade>
            <Fade>
            <Swiper
            breakpoints={{
                    320:{
                ...
                media.map(mediaItem => {
                    ...
                    return(
                        <SwiperSlide className='media_slider' key={mediaItem.id}>
                            <Link className='media_slider_link' to={`/${mediaType}/${mediaItem.id}`}>
                                <img className="media_slider_poster slide" src={`https://image.tmdb.org/t/p/w200${mediaItem.poster_path}`} alt={mediaItem.title}/>
                            </Link>
                            <p className='vote_average m-0'>{(mediaItem.vote_average / 10 * 100).toFixed(0)}<span className='vote_average_percentage'>%</span></p>
                            <p className='media_slider_title m-0'>{mediaType === 'movie' ? mediaItem.title : mediaItem.name}</p>
