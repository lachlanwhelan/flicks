    ...
    return (
      <div>
        ...
        {searchResults[0].results.map((searchItem) => {
          return (
            <Link
              to={`/${searchOption}/${searchItem.id}`}
              className='search_item_row d-flex align-items-center my-4'
              key={searchItem.id}
            >
                ...
                <img
                  src={
                    searchItem.poster_path
                      ? `https://image.tmdb.org/t/p/w200${searchItem.poster_path}`
                      : ImageNotFound
                  }
                />
              </div>
              ...
              <img
                src={
                  searchItem.profile_path
                    ? `https://image.tmdb.org/t/p/w200${searchItem.profile_path}`
                    : ImageNotFound
                }
              />
            </div>
  ...
  const getSearchResults = async () => {
    try {
      const responses = await Promise.all([
        fetch(
          `https://api.themoviedb.org/3/search/movie?api_key=2a5e286bb1bdc7c0bf49d7b6d0707880&language=en-US&query=${
            location.search.split('=')[1]
          }&page=1`
        ),
        fetch(
          `https://api.themoviedb.org/3/search/tv?api_key=2a5e286bb1bdc7c0bf49d7b6d0707880&language=en-US&query=${
            location.search.split('=')[1]
          }&page=1`
        ),
        fetch(
          `https://api.themoviedb.org/3/search/person?api_key=2a5e286bb1bdc7c0bf49d7b6d0707880&language=en-US&query=${
            location.search.split('=')[1]
          }&page=1`
        ),
      ]);
          ...
          <div className='col-md-3'>
            <div className='search_result_options'>
              ...
              <Button
                ...
                onClick={() => setSearchOption('movies')}
              >
                Movies{' '}
                <span className='d-inline-block ms-2'>
                  {searchResults[0].total_pages}
                </span>
              </Button>
              <Button
                variant='light'
