  ...
  *///if page number or props updates rerender component
  ...
  useEffect(() => {
    ...
    if(typeof prevProps !== 'undefined'){
        ...
        if(prevProps.category !== category || prevProps.mediaType !== mediaType){
            clearState();
        }
    }
    ...
    const getCategory = async () => {
        try{
            const responses = await Promise.all([
                fetch(`https://api.themoviedb.org/3/${mediaType}/${category}?api_key=2a5e286bb1bdc7c0bf49d7b6d0707880&language=en-US&page=${page}`),
                fetch(`https://api.themoviedb.org/3/genre/${mediaType}/list?api_key=2a5e286bb1bdc7c0bf49d7b6d0707880&language=en-US`)
            ]);
    ...
    const sortAndFilterSearch = (list) => {
        ...
        switch(parseInt(sortOption)){
           ...
           list.sort((a, b) => {
            ...
            case 3:
            list.sort((a, b) => {
                if(mediaType === 'movie') return new Date(a.release_date) - new Date (b.release_date);
            ...
            case 4: 
            list.sort((a, b) => {
                if(mediaType === 'movie') return new Date (b.release_date) - new Date(a.release_date) ;
           ...
           <div className='row'>
            <div className='col-md-3'>
                <div className='sort_form'>
                    <h3 className='mb-4 category_title title_accent'>{category.replace(/_/g, ' ')}</h3>
                    <Accordion defaultActiveKey="0" flush>
                        <Accordion.Item eventKey="0">
                            <Accordion.Header><b>Sort</b></Accordion.Header>
                            <Accordion.Body>
                            <label className='mb-2'>Sort results by</label>
                            <Form.Select aria-label="Default select example" value={sortOption} onChange={(e) => {setSortOption(e.target.value); console.log('test')}}>
                                    ...
                                    genres.map(genre => {
                                        return(
                                            <li className='m-1' key={genre.id}>
                                                <button className='genre_btn' onClick={handleGenreOnClick} value={genre.id} variant='outline-danger'>{genre.name}</button>
                                            </li>
                                        )
                                    })
                                }
                    ...
                    <Button onClick={handleSearchSubmit} variant='outline-danger w-100 my-2' size='lg'>Search</Button>
                </div>
            </div>
                            ...
                            media.map(mediaItem => {
                                ...
                                return(
                                    <div className='category_list_item'>
                                        <Link className='' to={`/${mediaType}/${mediaItem.id}`} key={mediaItem.id}>
