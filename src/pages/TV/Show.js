    ...
    const getMovieDetails = async () => {
        ...
        try{
            const responses = await Promise.all([
                fetch(`https://api.themoviedb.org/3/tv/${show_id}?api_key=2a5e286bb1bdc7c0bf49d7b6d0707880&language=en-US`),
                fetch(`https://api.themoviedb.org/3/tv/${show_id}/credits?api_key=2a5e286bb1bdc7c0bf49d7b6d0707880&language=en-US`),
                fetch(`https://api.themoviedb.org/3/tv/${show_id}/similar?api_key=2a5e286bb1bdc7c0bf49d7b6d0707880&language=en-US`),
                fetch(`https://api.themoviedb.org/3/tv/${show_id}/videos?api_key=2a5e286bb1bdc7c0bf49d7b6d0707880&language=en-US`)
            ])
           
            const show_details = await Promise.all(responses.map(response => {
                if(response.ok){
    ...
    return(
        <main>
            ...
            <section className='media_info_section container my-5'>
                <h3 className='media_title  mt-4'>{show.name} ({new Date(show.first_air_date).getFullYear()})</h3>
                <h6 className='mb-4'>
                    {show.genres.map(genre => <span className='me-2'>{genre.name}</span> )}
                </h6>
